...
/*
 * Initialization of shared memory for Data Item
 */
Size
DataItemShmemSize(void)
{
	Size		size;
	size	=	sizeof(DataItem);
	size	=   add_size(size, DATAITEM_SIZE * sizeof(int));
	return 		size;
}

/*
 * Initialization of shared memory for Data Matrix
 */

Size
DataMatrixShmemSize(void)
{
	Size		size;
	size	=   sizeof(DataMatrix);
	size	=   add_size(size, HASH_SIZE * sizeof(MarkovNode));
	return 		size;
}

/*
 * Initialization of shared memory for Write Matrix
 */

Size
WriteMatrixShmemSize(void)
{
	Size		size;
	size	=	sizeof(WriteMatrix);
	size	=   add_size(size, HASH_SIZE * sizeof(WriteNode));
	return 		size;
}

/*
 * Initialization of shared memory for Transaction Pool
 */
...
TxnPoolShmemSize(void)
{
	...
	size	=   add_size(size, TXN_SIZE * 2 * searchDepth * searchWidth * sizeof(int));
	/* size	=   add_size(size, TXN_SIZE * HASH_KEY_SIZE * sizeof(int));*/
	return 		size;
}

/*
 * Initialization of shared memory for Commit Transaction Pool
 */
...
/*
 * Initialization of shared memory for Shared Data
 */
Size
...
void 
updateDataItemFromFile(const char* filename) 
{
    ...
    while (fgets(line, sizeof(line), file)) 
	{
        ...
        const char* data_str 	= 	strtok(line, ":");
        const char* data_index 	= 	strtok(NULL, ":");
        
        if(data_str && data_index)
        {
			int 		data_content 	=	atoi(data_str);
			int			index			=	atoi(data_index) % DATAITEM_SIZE;
			int			temp 			=	index;
			
			while(dataItem->data_content[index] != -1 && dataItem->data_content[index] != 0)
			{
				if(dataItem->data_content[index] == data_content)
					break;
				index++;
				if(index == DATAITEM_SIZE)
					index = 0;
				if(index == temp)
					return;
			}
			dataItem->data_content[index] 	= 	data_content;

			/* printf("[INITDATA  Pid:%d] data_content[%d] = %d  \n", MyProcPid, index, data_content); */
		}
    }
    fclose(file);
}
...
/* Update dataMatrix from a txt file */
void updateDataMatrixFromFile(const char* filename) 
{ 
    FILE* file = fopen(filename, "r");
    ...
    while (fgets(line, sizeof(line), file)) 
	{
        ...
        char* data1_str = strtok(line, ":,");
        char* data2_str = strtok(NULL, ":,");
        char* data3_str = strtok(NULL, ":,");
		char* count_str = strtok(NULL, ":,");

        if (data1_str != NULL && data2_str != NULL && data3_str != NULL && count_str!=NULL) 
		{
            int data1 = atoi(data1_str);
			int data2 = atoi(data2_str);
			int data3 = atoi(data3_str);
            int count = atoi(count_str);
			
			int index =  (data1+data2) % HASH_SIZE;
			
			while(hashMatrix->matrix[index]->weight != -1)
			{
				
				if(hashMatrix->matrix[index]->data1 == data1 && hashMatrix->matrix[index]->data2 == data2)
				{
					if(hashMatrix->matrix[index]->data3 != data3)
					{
						hashMatrix->matrix[index]->isTheLast = false;
					}
					else
					{
						count += hashMatrix->matrix[index]->weight;
						break;
					}
				}
				...
				if(index == ((data1+data2) % HASH_SIZE))
					elog(ERROR, "Matrix Full!");
				
			}
			
			hashMatrix->matrix[index]->data1			=		data1;
			hashMatrix->matrix[index]->data2			=		data2;
			hashMatrix->matrix[index]->data3			=		data3;
            hashMatrix->matrix[index]->weight 			= 		count;
			hashMatrix->matrix[index]->isTheLast		=  		true;	
			
			if(index == HASH_SIZE-1)
			{
				if(hashMatrix->matrix[0]->weight != -1)
					hashMatrix->matrix[index]->next 	=		hashMatrix->matrix[0];

				if(hashMatrix->matrix[index-1]->weight != -1)
					hashMatrix->matrix[index-1]->next	=		hashMatrix->matrix[index];
			}
			else if(index == 0)
			{
				if(hashMatrix->matrix[index+1]->weight != -1)
					hashMatrix->matrix[index]->next 		=		hashMatrix->matrix[index+1];
				if(hashMatrix->matrix[HASH_SIZE-1]->weight != -1)
					hashMatrix->matrix[HASH_SIZE-1]->next	=		hashMatrix->matrix[index];
			}
			else 
			{
				if(hashMatrix->matrix[index+1]->weight != -1)
...
/* Update writeMatrix from a txt file */
void 
updateWriteMatrixFromFile(const char* filename) 
{
    FILE* file = fopen(filename, "r");
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        char* data_str = strtok(line, ":,");
        char* count_read_str = strtok(NULL, ":,");
        char* count_write_str = strtok(NULL, ":,");

        if (data_str != NULL && count_read_str != NULL && count_write_str != NULL) {
            int data = atoi(data_str);
            int count_read = atoi(count_read_str);
            int count_write = atoi(count_write_str);

			int index=  data % HASH_SIZE;

			while(writeMatrix->matrix[index]->data != -1)
			{
				index++;
				if(index == HASH_SIZE-1)
					index = 0;

				if(index == (data % HASH_SIZE))
					elog(ERROR, "Matrix Full!");
			}
			writeMatrix->matrix[index]->data			=		data;
            writeMatrix->matrix[index]->count_read 		= 		count_read;
            writeMatrix->matrix[index]->count_write 	= 		count_write;
            writeMatrix->matrix[index]->next 			=		NULL;
			
			if(index == HASH_SIZE-1)
			{
				if(writeMatrix->matrix[0]->data != -1)
					writeMatrix->matrix[index]->next 	=		writeMatrix->matrix[0];

				if(writeMatrix->matrix[index-1]->data != -1)
					writeMatrix->matrix[index-1]->next	=		writeMatrix->matrix[index];

			}
			else if(index == 0)
			{
				if(writeMatrix->matrix[index+1]->data != -1)
					writeMatrix->matrix[index]->next 		=		writeMatrix->matrix[index+1];

				if(writeMatrix->matrix[HASH_SIZE-1]->data != -1)
					writeMatrix->matrix[HASH_SIZE-1]->next	=		writeMatrix->matrix[index];
			}
			else 
			{
				if(writeMatrix->matrix[index+1]->data != -1)
...
{
	bool	foundDataItem;
	dataItem 					= 	(struct DataItem*)ShmemInitStruct("dataItem", DataItemShmemSize(), &foundDataItem);

	if(!foundDataItem)
	{
		for(int i=0; i<DATAITEM_SIZE; i++)
		{
			dataItem->data_content[i]	=	-1;	
		}
		
		const char* dataItemFile = "C:\\Users\\Administrator\\Desktop\\data.txt";
		updateDataItemFromFile(dataItemFile); 
	}
...
initDataMatrix(void)
{
	bool foundDataMatrix;
	hashMatrix = (struct DataMatrix*)ShmemInitStruct("hashMatrix", DataMatrixShmemSize(), &foundDataMatrix);
	if(!foundDataMatrix)
	{
		int i	=	0;
		for(i = 0; i < HASH_SIZE; i++)
		{
			MarkovNode* 	temp_matrix			=	(struct MarkovNode*)ShmemAlloc(sizeof(MarkovNode));
			hashMatrix->matrix[i] 				= 	temp_matrix;
			hashMatrix->matrix[i]->data1 		= 	-1;
			hashMatrix->matrix[i]->data2 		= 	-1;
			hashMatrix->matrix[i]->data3 		= 	-1;
			hashMatrix->matrix[i]->weight 		= 	-1;
			hashMatrix->matrix[i]->isTheLast	= 	false;
			hashMatrix->matrix[i]->next 		= 	NULL;
		}
		const char* dataMatrixFile = "C:\\Users\\Administrator\\Desktop\\matrix.txt";
		updateDataMatrixFromFile(dataMatrixFile); 
	}
}		

void 
initWriteMatrix(void)
{
	bool foundWriteMatrix;
	writeMatrix = (struct WriteMatrix*)ShmemInitStruct("writeMatrix", WriteMatrixShmemSize(), &foundWriteMatrix);
	if(!foundWriteMatrix)
	{
		int i	=	0;
		for(i=0; i<HASH_SIZE; i++)
		{
			writeMatrix->matrix[i]					= 	(struct WriteNode*)ShmemAlloc(sizeof(WriteNode));
			writeMatrix->matrix[i]->data 			= 	-1;
			writeMatrix->matrix[i]->count_read 		= 	-1;
			writeMatrix->matrix[i]->count_write 	= 	-1;
			writeMatrix->matrix[i]->next 			= 	NULL;
		}
		const char* writeMatrixFile = "C:\\Users\\Administrator\\Desktop\\write.txt";
		updateWriteMatrixFromFile(writeMatrixFile);
	}
}		
...
TransactionPoolInit(void)
{
	...
	{ 	
		...
		for(int i=0; i < TXN_SIZE; i++)
		{
			transactionPool->workingSet[i]				=	(TransactionWorkingSet*)ShmemAlloc(sizeof(TransactionWorkingSet));
			transactionPool->workingSet[i]->count_data	=	0;
			transactionPool->workingSet[i]->vxid.backendId			= 	InvalidBackendId;
			transactionPool->workingSet[i]->vxid.localTransactionId	= 	InvalidLocalTransactionId;
			transactionPool->workingSet[i]->workingset	=	(int*)ShmemAlloc((searchDepth) * (searchWidth) * sizeof(int));
			transactionPool->workingSet[i]->isWrite		=	(int*)ShmemAlloc((searchDepth) * (searchWidth) * sizeof(int));
			transactionPool->workingSet[i]->committed	=	false;
			transactionPool->workingSet[i]->next		=	NULL;
		}
	}
...
InitProcess(void)
{
	...
	/* OwnLatch(SharedData->myLatch);  New */
