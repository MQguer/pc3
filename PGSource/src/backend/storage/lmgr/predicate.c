...
void clearMySerializableXactAndGetSnapShot(Snapshot snapshot)
{
	...
	MySerializableXact->SeqNo.lastCommitBeforeSnapshot = PredXact->LastSxactCommitSeqNo;
	MySerializableXact->prepareSeqNo = InvalidSerCommitSeqNo;
	MySerializableXact->commitSeqNo = InvalidSerCommitSeqNo;
	SHMQueueInit(&(MySerializableXact->outConflicts));
	SHMQueueInit(&(MySerializableXact->inConflicts));
	SHMQueueInit(&(MySerializableXact->possibleUnsafeConflicts));
	MySerializableXact->topXid = GetTopTransactionIdIfAny();
	MySerializableXact->finishedBefore = InvalidTransactionId;
	MySerializableXact->xmin = snapshot->xmin;
	MySerializableXact->pid = MyProcPid;
	SHMQueueInit(&(MySerializableXact->predicateLocks));
	SHMQueueElemInit(&(MySerializableXact->finishedLink));
	MySerializableXact->flags = 0;
...
CheckForSerializableConflictOutNeeded(Relation relation, Snapshot snapshot)
{
	...
	if (SxactIsDoomed(MySerializableXact) && !pc3 )
	{
		ereport(ERROR,
				(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
