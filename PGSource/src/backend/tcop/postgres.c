...
struct TransactionWorkingSet* workingset = NULL;
 ...
 * ----------------------------------------------------------------
 */
extern void addWaitedVXID(VirtualTransactionId newVxid, int newPid);
extern void addWaitingVXID(VirtualTransactionId newVxid);
extern int* removeWaitedVxid(VirtualTransactionId vxidToRemove, int* pidSize);
extern void removeWaitingVXID(VirtualTransactionId vxidToRemove);
extern bool checkWaitingStatus(VirtualTransactionId vxid);
extern int pidCountDecrease(int pid);
...
/* Get data ID from dataItem, if no ID found, assign one */
int 
getDataIndex(const char* str) 
{
	if(strlen(str) <= 4)
	{
		if(boolPrint)
			printf("\n ----- [PID: %d] REPORT: str is less than 5 chars, str = %s. ----- \n", MyProcPid, str);
		return -1;
	}
	
	char* 	subStr 		= 	&str[4];
	int 	data		=	atoi(subStr);
	
    if (dataItem == NULL || str == NULL) 
	{
		if(boolPrint)
			printf("\n ----- [PID: %d] REPORT: dataItem or parmValue is NULL ----- \n", MyProcPid);
        return -1; 
    }

	int 	i 		= 	data % DATAITEM_SIZE;
	int		temp	=	i;

    while(dataItem->data_content[i] != -1 && dataItem->data_content[i] != 0)
	{
        if (dataItem->data_content[i] == data) 
		{	
            return i; 
        }
		else 
		{
			...
			{
				if(boolPrint)
					printf("\n +++++++++++ [PID: %d] REPORT ERROR : Full DataItem!  +++++++++++ \n", MyProcPid);
				return -1;
			}
		}
    }

	if(dataItem->data_content[i] == 0	||	dataItem->data_content[i] == -1)
	{
    	dataItem->data_content[i] 	= 	data;
		...
		{
			if(boolPrint)
				printf("\n +++++++++++ [PID: %d] REPORT ERROR : dataItem->data_content[i] = %d, but data = %d. +++++++++++ \n", MyProcPid, dataItem->data_content[i], data);
			return -1;
		}
	}
	else
	{
		if(boolPrint)
			printf("\n +++++++++++ [PID: %d] REPORT ERROR : dataItem->data_content[i] already occupied! +++++++++++ \n", MyProcPid);
		return -1;
	}
}
...
HashTableNode* 
initHashTable(HashTableNode* hashTable, int hashSize)
{
	hashTable = (HashTableNode*)malloc(hashSize * sizeof(HashTableNode));
	memset(hashTable, -1, hashSize * sizeof(HashTableNode));
	return hashTable;
}

void 
insertIntoHashTable(HashTableNode* hashTable, int hashSize, int data, int isWrite)
{
	int index	=	data % hashSize;

	while(hashTable[index].data != -1)
	{
		if(hashTable[index].data == data)
		{
			return;
		}
		...
		if(index == data % hashSize)
		{
			elog(ERROR, "hashTable Full!");
			return;		
		}
	}
	if(hashTable[index].data == -1)
	{
		...
		hashTable[index].isWrite = isWrite;
	}
	else 
	{
		return;
...
bool
findHashByData(HashTableNode* hashTable, int hashSize, int data, int isWrite)
{
	int index = data % hashSize;

	while(hashTable[index].data != -1)
	{
		if(hashTable[index].data == data && hashTable[index].isWrite + isWrite >= 1)  
		{
			return true;
		}
...
bool 
intersectNotEmpty(int* num1, int* isWrite1, int num1Size, int* num2, int* isWrite2, int num2Size)
{
	int hashSize = 50;

	HashTableNode* hashTable;
	hashTable = initHashTable(hashTable, hashSize);

	for(int i=0; i<num1Size; i++)
	{
		insertIntoHashTable(hashTable, hashSize, num1[i], isWrite1[i]);
	}
	
	for(int j=0; j<num2Size; j++)
	{
		if(findHashByData(hashTable, hashSize, num2[j], isWrite2[j]))
		{
			return true;
		}
	}	
...
int 
intersectNotEmptyForOCC(int* num1, int* isWrite1, int num1Size, int* num2, int* isWrite2, int num2Size)
{
	int hashSize 	= 	50;
...
int 
getRandomNumberByWeight(int *weights, int size) 
{
    int totalWeight = 0;
...
/* Make predictions */
void 
beam_search(int* predictedData, int* predictWrite, int* index_predict)
{	
	if(*index_predict < order_num || *index_predict == searchDepth)
	{
		return;
	}
	
	int data1 = predictedData[*index_predict-2];
	int data2 = predictedData[*index_predict-1];
	
	int *choice = (int *)malloc(sizeof(int));	
	int *weight = (int *)malloc(sizeof(int));	
	
	MarkovNode* ptrNode 	= 	hashMatrix->matrix[(data1+data2) % HASH_SIZE];
	int temp1 = ptrNode->data1;
	int temp2 = ptrNode->data2;
	int temp3 = ptrNode->data3;
	
	int index_choice = 0;
	
	bool flag = false;

	while(ptrNode->weight != -1)
	{
		if(ptrNode->data1 == data1 && ptrNode->data2 == data2)
		{
			index_choice = index_choice + 1;
			choice =  (int*)realloc(choice, index_choice * sizeof(int));
			
			choice[index_choice-1] = ptrNode->data3;
			weight[index_choice-1] = ptrNode->weight;
			
			if(ptrNode->isTheLast)
				break;
		}
		ptrNode = ptrNode->next;
		
		if(ptrNode->data1 == temp1 && ptrNode->data2 == temp2 && ptrNode->data3 == temp3)
			break;
	}
	
	if(searchDepth > *index_predict && index_choice > 0)
	{	
		int chosen_data_index = getRandomNumberByWeight(weight, index_choice);

		if(chosen_data_index == -1)
		{
			elog(ERROR, "Wrong Data Index!");
			return;
		}
		int data3 = choice[chosen_data_index];

		int write_choice[2] = {0,1};
		int write_weight[2] = {1,1};
		
		WriteNode* ptr_writeNode = writeMatrix->matrix[data3 % HASH_SIZE];
		
		int temp = ptr_writeNode->data;

		while(ptr_writeNode->data != -1)
		{	
			if(ptr_writeNode->data == data3)
			{
				write_weight[0] = ptr_writeNode->count_read;
				write_weight[1] = ptr_writeNode->count_write;
				break;
			}
			
			ptr_writeNode = ptr_writeNode->next;
			
			if(ptr_writeNode->data == temp)
				break;	
		}
  
		int chosen_write_index = getRandomNumberByWeight(write_weight, 2);

		if(chosen_write_index == -1)
		{
			elog(ERROR, "Wrong Write Index!");
			return;
		}
		
		int is_write = write_choice[chosen_write_index];
			
		predictedData[*index_predict] 	= 	data3;
		predictWrite[*index_predict] 	= 	is_write;
		
		*index_predict = *index_predict + 1;
		
		beam_search(predictedData, predictWrite, index_predict);
		
	}
	else if(index_choice == 0)
	{
		return;
	}
}
...
addNewTransactionToPool(VirtualTransactionId current_vxid)
{
	...
	int i = hash_new_index;

	int k = 0;

	while(transactionPool->workingSet[i]->vxid.backendId > 0 && transactionPool->workingSet[i]->vxid.localTransactionId > 0)
	{
		if(ssn)
		{
			int random = rand() % 4;
		...
		if( equalVXID(current_vxid, transactionPool->workingSet[i]->vxid) )
		{
			return i;
		}
	...
	if( transactionPool->workingSet[i]->vxid.backendId <= 0 || transactionPool->workingSet[i]->vxid.localTransactionId <= 0 )
	{
		transactionPool->workingSet[i]->vxid		=	current_vxid;		
		transactionPool->workingSet[i]->count_data	=	0;
		transactionPool->workingSet[i]->begin_time	=	time(NULL);
		
		if(equalVXID(current_vxid, transactionPool->workingSet[i]->vxid))
		{
			while(ptr_txn->next)
				ptr_txn = ptr_txn->next;
			ptr_txn->next = transactionPool->workingSet[i];
			return i;
		}
		else 
		{
			if(boolPrint) 
				printf("\n ------ [Pid: %d] REPORT：Error in (vxid[i] != current_vxid), vxid[i] = %d/%d ----- \n", MyProcPid,  transactionPool->workingSet[i]->vxid.backendId, transactionPool->workingSet[i]->vxid.localTransactionId);
			return -1;	
		}
	}
	else 
	{
		if(boolPrint)
			printf("\n ------ [Pid: %d] REPORT：Error in (vxid[i] != INVALID), VXID = %d/%d ----- \n", MyProcPid, transactionPool->workingSet[i]->vxid.backendId, transactionPool->workingSet[i]->vxid.localTransactionId);
		return -1;
	}
...
{
	int*	my_workingset 	= 	currentTxn->workingset;
	int*	my_writeset		=	currentTxn->isWrite;
	int 	my_txn_size 	=	currentTxn->count_data;

	int*	current_wokingset 	= 	NULL;
	int*	current_writeset 	= 	NULL;
	int 	current_size 		= 	0;
	...
	while(ptr_transaction)
	{
		if( !equalVXID( ptr_transaction->vxid,  current_vxid) && (ptr_transaction->vxid.backendId > 0) && (ptr_transaction->vxid.localTransactionId > 0) && ptr_transaction->begin_time != 0 )
		{
			if( ptr_transaction->committed && ptr_transaction->commit_time > currentTxn->begin_time && ptr_transaction->begin_time < currentTxn->commit_time)
			{
				...
				current_writeset 	= 	ptr_transaction->isWrite;
				current_size		=	ptr_transaction->count_data;
	...
	*resultSize					=	0;
	...
	while(ptr_transaction)
	{
		if( !equalVXID( ptr_transaction->vxid,  current_vxid) && (ptr_transaction->vxid.backendId > 0) && (ptr_transaction->vxid.localTransactionId > 0) )
		{
			...
			bool foundConflict 	= 	intersectNotEmpty(my_workingset, my_writeset, my_txn_size, current_wokingset, current_writeset, current_size);
...
void
findConflictsForSSN( VirtualTransactionId current_vxid, int data, int isWrite)
{
	int*	current_wokingset 	= 	NULL;
	int*	current_writeset 	= 	NULL;
	int 	current_size 		= 	0;
	...
	while(ptr_transaction)
	{
		if( !equalVXID( ptr_transaction->vxid, current_vxid) && (ptr_transaction->vxid.backendId > 0) && (ptr_transaction->vxid.localTransactionId > 0) )
		{
			...
			{
				if(current_wokingset[i] == data)
				{
					if( current_writeset[i] == 1 && isWrite == 1 )
					{
						addConflictRecord(ptr_transaction->vxid,  current_vxid, Conf_WW);
					}
					else if( current_writeset[i] == 0 && isWrite == 1 )
					{
						addConflictRecord(ptr_transaction->vxid,  current_vxid, Conf_RW);
					}
					else if( current_writeset[i] == 1 && isWrite == 0 )
					{
...
exec_bind_message(StringInfo input_message)
{
	...
	*/
	...
	const char*		parmValue 			= 	NULL; 
	CmdType 		query_type 			= 	getSqlStatementType(sql_statement);
	VirtualTransactionId current_vxid 	= 	getVirtualXid();

	if(!pcc && query_type!=CMD_NOTHING && query_type!=CMD_UNKNOWN && query_type!=CMD_INSERT)
	{
   ...
   if (numParams > 0)
   {
	...
	if(pc3 && typeFlag)
	{
		...
		if(query_type == CMD_UTILITY)
		{
			int*	pidToWake;
			int		wakeSize	=		0;
			pidToWake			=		removeWaitedVxid(current_vxid, &wakeSize);
			...
			removeWaitingVXID(current_vxid);
		...
		{
			int 	data_item 	=	-1;
			int		isWrite 	= 	(query_type == CMD_SELECT ? 0 : 1);

			if(parmValue)
			{
				data_item = getDataIndex(parmValue);
			}

			if(data_item >= 0 && index_current_txn >= 0)
			{
				int 	new_data_index 		= 	transactionPool->workingSet[index_current_txn]->count_data;

				if(new_data_index < searchDepth * searchWidth && new_data_index >= 0)
				{
					if( transactionPool->workingSet[index_current_txn]->count_data <= 1)
					{
						transactionPool->workingSet[index_current_txn]->workingset[new_data_index]	=	data_item;
						transactionPool->workingSet[index_current_txn]->isWrite[new_data_index]		=	isWrite;
						transactionPool->workingSet[index_current_txn]->count_data++;
					} 

					VirtualTransactionId* 	result 					=  	NULL;
					int						resultSize 				= 	0;

					if( transactionPool->workingSet[index_current_txn]->count_data == 2)
					{
						beam_search(transactionPool->workingSet[index_current_txn]->workingset, transactionPool->workingSet[index_current_txn]->isWrite, &transactionPool->workingSet[index_current_txn]->count_data); 
						result = findConflictedTxn(result, &resultSize, current_vxid, transactionPool->workingSet[index_current_txn]);
					}
					...
					{   
					   	...
					   	for (int t=0; t<resultSize; t++)
					   	{
						   	...
						   	{
						   		addWaitedVXID(result[t], MyProcPid, current_vxid);
								addWaitingVXID(current_vxid, result[t]);
						   	}
					   	}
						
						/* needsWaiting */
						int level = checkTxnLevel(current_vxid);
					   	...
					   	{	
							if(boolPrint)
							{
						   	...
						   	ProcWaitForSharedSignal(PG_WAIT_SHARE, MyProcPid, 100);
	...
	if(ssn && typeFlag)
	{
		...
		else
		{
			...
			/*
			if(boolPrint && (data_item == -1 || index_current_txn == -1))
			{
				printf("\n------------- [Pid:%d] REPORT ERROR：data_item = %d, index_current_txn = %d -------------\n", MyProcPid, data_item, index_current_txn);
			}*/

			if(index_current_txn >= 0 && data_item >= 0)
			{
				int 	new_data_index 		= 	transactionPool->workingSet[index_current_txn]->count_data;
				/*
				if(boolPrint && (new_data_index >= searchDepth * searchWidth && boolPrint || new_data_index < 0))
				{
					printf("\n------------- [Pid:%d] REPORT ERROR：transaction workset error! new_data_index = %d  -------------\n", MyProcPid, new_data_index);
				}*/
				...
				if(new_data_index < searchDepth * searchWidth && new_data_index >= 0)
				{
					...
					findConflictsForSSN(current_vxid, data_item, isWrite);
	...
	if(occ && typeFlag)
	{
		...
		if(query_type == CMD_UTILITY)
		{
			...
			int conflict_num = findConflictedTxnForOCC(current_vxid, transactionPool->workingSet[index_current_txn]);
		...
		else
		{
			...
			if(boolPrint && (data_item == -1 || index_current_txn == -1))
			{
				elog(ERROR, "data_item == -1 OR index_current_txn == -1");
			}
			*/
			if(index_current_txn >= 0 && data_item >= 0)
			{
			...
			case 'P':			/* parse */
				{
					...
					if( pcc )
					{	
						CmdType query_type = getSqlStatementType(query_string);
						
						if(query_type == CMD_SELECT)
						{
							const char*	suffix = " FOR SHARE";
							
							int new_length = strlen(query_string) + strlen(suffix) + 1;

							const char *sql_statement = (const char*)malloc(new_length * sizeof(char));

							strcpy(sql_statement, query_string);
							strcat(sql_statement, suffix);

							exec_parse_message(sql_statement, stmt_name,
